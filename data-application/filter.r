...
updateStep = function(obs, appr, forecast_mean, forecast_covmodel, lik.params, saveUQ) {
    ...
    tryCatch({
        preds.aux = GPvecchia::calculate_posterior_VL(obs, appr, prior_mean = forecast_mean,
                                                      ...
                                                      max.iter=1000)
        if (!preds.aux$cnvgd) {
            warning("Vecchia Filter: Posterior estimation did not converge")
        }
...
VecchiaFilter = function(appr, Y, lik.params, prior_covparms, Qcovparms, evolFun,
    ...
    cat(sprintf("%s Vecchia Filter: Filtering started\n", Sys.time()))
    if (is.null(appr) | is(appr, "character")) {
        stop("Vecchia Filter: No vecchia approximation provided")
    }
    ...
    for (t in 1:Tmax) {
        ...
        cat(sprintf("%s \t Vecchia Filter: Filtering t=%d\n", Sys.time(), t))
