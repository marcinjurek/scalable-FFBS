...
smoothedMeans = function(approx, Y, lik.params, prior_covparams, Qcovparms, evolFun, wDir, verbose = FALSE) {

    preds = VecchiaReusableFilter(approx, Y, lik.params, prior_covparams, Qcovparms, evolFun,
                        saveUQ = c("L", "V"), wDir = wDir)
    n = length(Y[[1]])
    Tmax = length(Y)
    means = list()
